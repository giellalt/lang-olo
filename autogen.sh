...
function print_usage() {
    ...
    echo "  -l, --add-langvar   Add env. variable \$GTLANG_olo to login script."
...
if test x$langvar = xlangvar ; then
	if test x$LOGINFILE = x ; then
		...
		if test x$GTLANG_olo = x$LANGDIR ; then
			echo "\$GTLANG_olo already defined."
		else
			# Already defined with a different value:
			if test x$GTLANG_olo != x ; then
				renew=renew
				OLDLANGDIR=$GTLANG_olo
			fi

			# Add the variable to the login script:
			cp $LOGINFILE $LOGINFILE.gtbackup.${DATE}-${TIME}
			echo "export GTLANG_olo=$LANGDIR" >> $LOGINFILE
			source $LOGINFILE

			# Feedback depending on whether it was added or redefined:
			if test x$renew = xrenew ; then
			 echo "The env. variable \$GTLANG_olo has been redefined in"
			 echo "$LOGINFILE. The old value $OLDLANGDIR is no longer in use."
			else
			 echo "The env. variable \$GTLANG_olo has been added to $LOGINFILE."
...
# the current dir:
if   test x$GTLANG_olo = x ; then
	echo "WARNING: The variable \$GTLANG_olo has not been defined. You will not"
	...
	echo "$0 -l"
	echo
elif test x$GTLANG_olo != x$LANGDIR ; then
	echo "WARNING: The variable \$GTLANG_olo has the value:"
	echo "  $GTLANG_olo"
